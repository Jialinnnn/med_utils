...
def parse_args():
    ...
    # parser.add_argument('-o', '--out-dir',default='../data/coco', help='output path')
    parser.add_argument(
        '--out-format',
        default='coco',
        choices=('pkl', 'coco'),
...
def main():
    ...
    data_root = os.path.abspath(os.path.join(os.getcwd(), "../.."))  # 返回上层目录
    devkit_path = data_root + "/data/VOCdevkit"  # 数据集路径
    # devkit_path = '../data/VOCdevkit'
    # out_dir = args.out_dir if args.out_dir else devkit_path
    out_dir = data_root + "/data/coco"  # 数据集路径
...
#
# data_root = os.path.abspath(os.path.join(os.getcwd(), "../.."))  # 返回上层目录
# devkit_path = data_root + "/data/VOCdevkit/VOC2007"  # 数据集路径
# txt_path = path.join(devkit_path, 'ImageSets/Main/')
# jpg_path = path.join(devkit_path, 'JPEGImages/')
# splits = ['train', 'val', 'test']
#
# for split in splits:
#     print('copy images by', split)
#     src_path = txt_path + split + '.txt'
#     save_path = path.join(data_root , "data/coco", split)
